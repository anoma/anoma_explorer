defmodule AnomaExplorerWeb.CompliancesLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> RealtimeHelpers.init_realtime(:compliances, @default_filters)}
  end
  ...
  def handle_info(:check_connection, socket) do
    if Client.configured?() do
      case Client.test_connection() do
        {:ok, _} ->
          socket = load_compliances(socket)
  ...
  @impl true
  def handle_info({:new_items_available, :compliances, meta}, socket) do
  ...
  defp loading_skeleton(assigns) do
    ~H"""
    <.loading_blocks message="Loading compliances..." class="py-12" />
    """
  end
